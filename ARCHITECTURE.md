...
- **边界限制**：视口不可滑出数据范围（DATA_MIN~DATA_MAX），允许20%视口宽度弹性余量
...
- **实现**：
  ...
  - 边界计算：`maxOX = -DATA_MIN*ppy + vw*0.2`（右边界），`minOX = vw*0.8 - DATA_MAX*ppy`（左边界）
  - 允许 20% 视口宽度的弹性余量
- **应用位置**：所有修改 `offsetX` 的地方（拖动、缩放、惯性、导航、键盘）
- **验证方法**：向左滑到最早年份（夏朝前2070年）应自动停止，向右滑到最晚年份（2026年）应自动停止
