...
### 6. 密度星级制（v2.0 重构）
- **机制**：密度滑块1-5星整数，默认5星（全部显示）。`densityLevel`变量控制事件过滤：`event.level <= densityLevel` 才显示
- **映射**：5星=全显(level 1-5), 4星=隐藏level 5, 3星=隐藏level 4-5, 2星=隐藏level 3-4-5, 1星=只显level 1
- **显示**：密度值用 `★`/`☆` 表示（如 `★★★☆☆`），不用数字倍数
- **验证方法**：拉低密度时，低星级小圆点应消失；拉高密度时恢复显示

### 7. 缩放最小值限制（v2.0 新增）
- **机制**：最小 `pixelsPerYear = (vw - 60) / (DATA_MAX - DATA_MIN)`，确保边界约束 maxOX >= minOX 始终成立
- **原因**：旧min=0.01时 maxOX < minOX 导致边界约束矛盾，clamping来回跳动产生突变
- **应用位置**：pinch缩放、wheel缩放、键盘缩放、朝代跳转
- **验证方法**：持续缩小直到不能再缩，此时边界不会跳动；数据范围略大于视口可小幅滑动

### 8. 选中事件突出显示（v2.0 新增）
- **机制**：通过 `.event-node.active` CSS类控制选中态
- **样式**：
  - stem加粗：`width:3px; opacity:0.8`
  - dot放大：`transform: scale(1.4)`
  - card边框加粗：`border-width:2px` + 增强box-shadow
- **恢复**：取消选中时（expandedCard变为其他或null）自动移除 `.active` 类，恢复默认样式
- **验证方法**：点击事件卡片展开后，stem和dot明显比其他事件粗/大；点击空白后恢复

### 9. 圆点大小等级差异（v2.0 调整）
- **尺寸**：level-1=22px, level-2=15px, level-3=9px, level-4=5px, level-5=3px
- **目的**：肉眼能明显区分事件重要程度，1星最大最亮，5星最小最暗
- **验证方法**：在缩放等级3+时，不同level的圆点大小应有明显视觉差异